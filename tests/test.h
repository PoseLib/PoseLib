...
#define REQUIRE_M(COND, MSG) if(!(COND)) { std::cout << "Failure: "#COND" was not satisfied. (" + MSG + ").\n"; return false; }
#define REQUIRE_EQ_M(VAL1,VAL2,MSG) {auto _v1 = VAL1; auto _v2 = VAL2; if(_v1 != _v2) { std::cout << "Failure: "#VAL1" was not equal to "#VAL2". (" << _v1 << " vs. " << _v2 <<  ") (" + MSG + ")\n"; return false; }}
#define REQUIRE_SMALL_M(VAL,THR,MSG) {auto _v1 = VAL; auto _v2 = THR; if(std::isnan(_v1) || std::abs(_v1) > _v2) { std::cout << "Failure: "#VAL" was not small "#THR". (" << _v1 << " vs. " << _v2 <<  ") (" + MSG + ")\n"; return false; }}
