...
void setup_scene(int Ncam, int N, CameraPose &pose, std::vector<std::vector<Point2D>> &x,
                 std::vector<std::vector<Point3D>> &X, std::vector<CameraPose> &cam_ext, Camera &cam0, std::vector<Camera> &cam_int, std::vector<std::vector<double>> &weights) {
...
std::vector<Test> register_optim_gen_absolute_test() {
    return {
        TEST(test_gen_absolute_pose_normal_acc),
        TEST(test_gen_absolute_pose_jacobian),
        TEST(test_gen_absolute_pose_jacobian_cameras),
        TEST(test_gen_absolute_pose_refinement),
        TEST(test_gen_absolute_pose_weighted_refinement),
        TEST(test_gen_absolute_pose_cameras_refinement)
    };
}
