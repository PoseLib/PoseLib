...
bool check_project_unproject(Camera camera, const Eigen::Vector2d &xp) {
    ...
    REQUIRE_SMALL_M((xp - xp2).norm(), 1e-6,
                    camera.model_name() + " (xp=" + to_string(xp.transpose()) + ", x=" + to_string(x.transpose()) +
                        ", xp2=" + to_string(xp2.transpose()) + ")");
    return true;
}
...
bool check_proj_jacobian(Camera camera, const Eigen::Vector2d &xp) {
    // Unproject
    Eigen::Vector3d x;
    camera.unproject(xp, &x);
...
std::vector<Test> register_camera_models_test() {
    return {TEST(test_id_from_string), TEST(test_id_string_converstion),
            TEST(test_model_name),     TEST(test_from_txt),
            TEST(test_to_txt),         TEST(test_project_unproject),
            TEST(test_jacobian),       TEST(test_jacobian_1D_radial)};
}