...
static std::vector<CameraPose> p3p_wrapper(const std::vector<Eigen::Vector3d> &x, const std::vector<Eigen::Vector3d> &X) {
    std::vector<CameraPose> output;
    p3p(x, X, &output);
    return output;
}

static std::vector<CameraPose> gp3p_wrapper(const std::vector<Eigen::Vector3d> &p, const std::vector<Eigen::Vector3d> &x,
...
static std::pair<std::vector<CameraPose>, std::vector<double>> gp4ps_kukelova_wrapper(const std::vector<Eigen::Vector3d> &p,
                                                                               const std::vector<Eigen::Vector3d> &x,
                                                                               const std::vector<Eigen::Vector3d> &X,
                                                                               bool filter_solutions) {
    std::vector<CameraPose> output;
...
static std::pair<std::vector<CameraPose>, std::vector<double>> gp4ps_camposeco_wrapper(const std::vector<Eigen::Vector3d> &p,
                                                                                const std::vector<Eigen::Vector3d> &x,
                                                                                const std::vector<Eigen::Vector3d> &X) {
    std::vector<CameraPose> output;
    std::vector<double> output_scales;
...
static std::pair<std::vector<CameraPose>, std::vector<double>> p4pf_wrapper(const std::vector<Eigen::Vector2d> &x,
                                                                     const std::vector<Eigen::Vector3d> &X,
                                                                     bool filter_solutions) {
    std::vector<CameraPose> output;
...
static std::vector<CameraPose> p2p2pl_wrapper(const std::vector<Eigen::Vector3d> &xp, const std::vector<Eigen::Vector3d> &Xp,
...
static std::vector<CameraPose> p6lp_wrapper(const std::vector<Eigen::Vector3d> &l, const std::vector<Eigen::Vector3d> &X) {
    std::vector<CameraPose> output;
    p6lp(l, X, &output);
    return output;
}
...
static std::vector<CameraPose> p2p1ll_wrapper(const std::vector<Eigen::Vector3d> &xp, const std::vector<Eigen::Vector3d> &Xp,
...
static std::vector<CameraPose> p1p2ll_wrapper(const std::vector<Eigen::Vector3d> &xp, const std::vector<Eigen::Vector3d> &Xp,
                                       const std::vector<Eigen::Vector3d> &l, const std::vector<Eigen::Vector3d> &X,
                                       const std::vector<Eigen::Vector3d> &V) {
    std::vector<CameraPose> output;
...
static std::vector<CameraPose> p3ll_wrapper(const std::vector<Eigen::Vector3d> &l, const std::vector<Eigen::Vector3d> &X,
                                     const std::vector<Eigen::Vector3d> &V) {
    std::vector<CameraPose> output;
    p3ll(l, X, V, &output);
    return output;
...
static std::vector<CameraPose> up2p_wrapper(const std::vector<Eigen::Vector3d> &x, const std::vector<Eigen::Vector3d> &X) {
    std::vector<CameraPose> output;
    up2p(x, X, &output);
    return output;
}

static std::vector<CameraPose> ugp2p_wrapper(const std::vector<Eigen::Vector3d> &p, const std::vector<Eigen::Vector3d> &x,
                                      const std::vector<Eigen::Vector3d> &X) {
    std::vector<CameraPose> output;
    ugp2p(p, x, X, &output);
    return output;
...
static std::vector<CameraPose> up1p2pl_wrapper(const std::vector<Eigen::Vector3d> &xp, const std::vector<Eigen::Vector3d> &Xp,
                                        const std::vector<Eigen::Vector3d> &x, const std::vector<Eigen::Vector3d> &X,
                                        const std::vector<Eigen::Vector3d> &V) {
    std::vector<CameraPose> output;
...
static std::vector<CameraPose> up4pl_wrapper(const std::vector<Eigen::Vector3d> &x, const std::vector<Eigen::Vector3d> &X,
                                      const std::vector<Eigen::Vector3d> &V) {
    std::vector<CameraPose> output;
    up4pl(x, X, V, &output);
    return output;
...
static std::vector<CameraPose> ugp4pl_wrapper(const std::vector<Eigen::Vector3d> &p, const std::vector<Eigen::Vector3d> &x,
...
static std::vector<MonoDepthImagePair> shared_focal_monodepth_relpose_3pt_wrapper(const std::vector<Eigen::Vector3d> &x1,
                                                                           const std::vector<Eigen::Vector3d> &x2,
                                                                           const std::vector<double> &d1,
                                                                           const std::vector<double> &d2) {
...
static std::vector<MonoDepthImagePair> varying_focal_monodepth_relpose_3pt_wrapper(const std::vector<Eigen::Vector3d> &x1,
                                                                            const std::vector<Eigen::Vector3d> &x2,
                                                                            const std::vector<double> &d1,
                                                                            const std::vector<double> &d2) {
    std::vector<MonoDepthImagePair> output;
...
void register_solvers(py::module &m) {
    ...
    m.def("gp3p", &gp3p_wrapper, py::arg("p"), py::arg("x"), py::arg("X"),
    ...
    m.def("p5lp_radial", &p5lp_radial_wrapper, py::arg("l"), py::arg("X"),
          py::call_guard<py::gil_scoped_release>());
    ...
    m.def("p1p2ll", &p1p2ll_wrapper, py::arg("xp"), py::arg("Xp"), py::arg("l"), py::arg("X"), py::arg("V"),
          py::call_guard<py::gil_scoped_release>());
    m.def("p3ll", &p3ll_wrapper, py::arg("l"), py::arg("X"), py::arg("V"),
          py::call_guard<py::gil_scoped_release>());
    m.def("up2p", &up2p_wrapper, py::arg("x"), py::arg("X"), py::call_guard<py::gil_scoped_release>());
    m.def("ugp2p", &ugp2p_wrapper, py::arg("p"), py::arg("x"), py::arg("X"),
          py::call_guard<py::gil_scoped_release>());
    ...
    m.def("up1p2pl", &up1p2pl_wrapper, py::arg("xp"), py::arg("Xp"), py::arg("x"), py::arg("X"), py::arg("V"),
          py::call_guard<py::gil_scoped_release>());
    m.def("up4pl", &up4pl_wrapper, py::arg("x"), py::arg("X"), py::arg("V"),
          py::call_guard<py::gil_scoped_release>());
    ...
    m.def("shared_focal_relpose_6pt", &shared_focal_relpose_6pt_wrapper, py::arg("x1"), py::arg("x2"),
          py::call_guard<py::gil_scoped_release>());
    m.def("shared_focal_monodepth_pose_3pt", &shared_focal_monodepth_relpose_3pt_wrapper, py::arg("x1"),
    ...
    m.def("varying_focal_monodepth_pose_4pt", &varying_focal_monodepth_relpose_3pt_wrapper, py::arg("x1"),
          py::arg("x2"), py::arg("d1"), py::arg("d2"), py::call_guard<py::gil_scoped_release>());
    m.def("relpose_5pt", &relpose_5pt_wrapper, py::arg("x1"), py::arg("x2"),
          py::call_guard<py::gil_scoped_release>());
    ...
    m.def("relpose_8pt", &relpose_8pt_wrapper, py::arg("x1"), py::arg("x2"),
          py::call_guard<py::gil_scoped_release>());
    ...
    m.def("relpose_upright_3pt", &relpose_upright_3pt_wrapper, py::arg("x1"), py::arg("x2"),
          py::call_guard<py::gil_scoped_release>());
    m.def("gen_relpose_upright_4pt", &gen_relpose_upright_4pt_wrapper, py::arg("p1"), py::arg("x1"),
          py::arg("p2"), py::arg("x2"), py::call_guard<py::gil_scoped_release>());
    m.def("gen_relpose_6pt", &gen_relpose_6pt_wrapper, py::arg("p1"), py::arg("x1"), py::arg("p2"),
