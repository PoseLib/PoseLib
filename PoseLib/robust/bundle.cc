...
BundleStats bundle_adjust(const std::vector<Point2D> &points2D, const std::vector<Point3D> &points3D,
    ...
    LossFunction loss_fn(opt.loss_scale);
    IterationCallback callback = setup_callback(opt, loss_fn);    
    PointLineJacobianAccumulator<LossFunction, PointWeightType, LineWeightType>
            accum(points2D, points3D, lines2D, lines3D, loss_fn, loss_fn, weights_pts, weights_lines);
    return lm_impl<decltype(accum)>(accum, pose, opt, callback);
}
...
#define SWITCH_LOSS_FUNCTION_CASE(LossFunction)                              \
        return bundle_adjust<PointWeightType, LineWeightType, LossFunction>( \
            points2D,points3D,lines2D,lines3D,pose,opt,weights_pts, weights_lines);
        SWITCH_LOSS_FUNCTIONS           
        default:
            return BundleStats();
    }
...
BundleStats bundle_adjust(const std::vector<Point2D> &points2D, const std::vector<Point3D> &points3D,
    ...
    if(have_pts_weights && have_line_weights) {
        ...
        return bundle_adjust<std::vector<double>, UniformWeightVector>(
            points2D,points3D,lines2D,lines3D,pose,opt,weights_pts, UniformWeightVector());
    } else if(!have_pts_weights && have_line_weights) {
        return bundle_adjust<UniformWeightVector, std::vector<double>>(
            points2D,points3D,lines2D,lines3D,pose,opt,UniformWeightVector(),weights_lines);
    } else {
        return bundle_adjust<UniformWeightVector, UniformWeightVector>(
            points2D,points3D,lines2D,lines3D,pose,opt,UniformWeightVector(),UniformWeightVector());
    }
}
...
// Interface for calibrated camera
BundleStats generalized_bundle_adjust(const std::vector<std::vector<Point2D>> &x, const std::vector<std::vector<Point3D>> &X,
                                      const std::vector<CameraPose> &camera_ext, CameraPose *pose, const BundleOptions &opt,
...
BundleStats generalized_bundle_adjust(const std::vector<std::vector<Point2D>> &x, const std::vector<std::vector<Point3D>> &X,
    ...
    if(weights.size() == x.size()) {
        return generalized_bundle_adjust<std::vector<std::vector<double>>>(x,X,camera_ext,cameras,pose,opt,weights);
    } else {
        return generalized_bundle_adjust<UniformWeightVectors>(x,X,camera_ext,cameras,pose,opt,UniformWeightVectors());
    }
}
...
BundleStats refine_generalized_relpose(const std::vector<PairwiseMatches> &matches,
    ...
    LossFunction loss_fn(opt.loss_scale);
    IterationCallback callback = setup_callback(opt, loss_fn);
    GeneralizedRelativePoseJacobianAccumulator<LossFunction, WeightType>
...
BundleStats refine_generalized_relpose(const std::vector<PairwiseMatches> &matches,
    ...
    if (weights.size() == matches.size()) {
        return refine_generalized_relpose<std::vector<std::vector<double>>>(matches, camera1_ext, camera2_ext, pose, opt, weights);
    } else {
        return refine_generalized_relpose<UniformWeightVectors>(matches, camera1_ext, camera2_ext, pose, opt, UniformWeightVectors());
    }
}
...
BundleStats refine_hybrid_pose(const std::vector<Point2D> &x, const std::vector<Point3D> &X,
    ...
    HybridPoseJacobianAccumulator<LossFunction, AbsWeightType, RelWeightType>
        accum(x, X, matches_2D_2D, map_ext, loss_fn, loss_fn_epipolar, weights_abs, weights_rel);
    return lm_impl<decltype(accum)>(accum, pose, opt, callback);
}
...
BundleStats refine_hybrid_pose(const std::vector<Point2D> &x, const std::vector<Point3D> &X,
    ...
    switch (opt.loss_type) {        
#define SWITCH_LOSS_FUNCTION_CASE(LossFunction) \
        return refine_hybrid_pose<AbsWeightType,RelWeightType,LossFunction>(x,X,matches_2D_2D,map_ext,pose,opt,loss_scale_epipolar,weights_abs,weights_rel);
        SWITCH_LOSS_FUNCTIONS           
        default:
            return BundleStats();
    }
...
BundleStats refine_hybrid_pose(const std::vector<Point2D> &x, const std::vector<Point3D> &X,
    ...
    if(have_abs_weights && have_rel_weights) {
        ...
        return refine_hybrid_pose<UniformWeightVector, UniformWeightVectors>(
                x,X,matches_2D_2D,map_ext,pose,opt,loss_scale_epipolar,UniformWeightVector(),UniformWeightVectors());
    }
}
