...
size_t CameraJacobianAccumulatorSIMD<NullCameraModel, TruncatedLoss, UniformWeightVector>::accumulate(const CameraPose &pose, Eigen::Matrix<double, 6, 6> &JtJ,
        ...
        for (size_t k = 0; k < K - 3; k += 4) {
            ...
            const __m256d r[2]{_mm256_sub_pd(z[0], _mm256_loadu_pd(x_p + k)), _mm256_sub_pd(z[1], _mm256_loadu_pd(x_p + K + k))};
            ...
            const __m256d dZtdZ_0_0 = _mm256_and_pd(valid_mask, _mm256_add_pd(_mm256_mul_pd(dZ[0][0], dZ[0][0]), _mm256_mul_pd(dZ[1][0], dZ[1][0])));
            const __m256d dZtdZ_1_0 = _mm256_and_pd(valid_mask, _mm256_add_pd(_mm256_mul_pd(dZ[0][1], dZ[0][0]), _mm256_mul_pd(dZ[1][1], dZ[1][0])));
            const __m256d dZtdZ_1_1 = _mm256_and_pd(valid_mask, _mm256_add_pd(_mm256_mul_pd(dZ[0][1], dZ[0][1]), _mm256_mul_pd(dZ[1][1], dZ[1][1])));
            const __m256d dZtdZ_2_0 = _mm256_and_pd(valid_mask, _mm256_add_pd(_mm256_mul_pd(dZ[0][2], dZ[0][0]), _mm256_mul_pd(dZ[1][2], dZ[1][0])));
            const __m256d dZtdZ_2_1 = _mm256_and_pd(valid_mask, _mm256_add_pd(_mm256_mul_pd(dZ[0][2], dZ[0][1]), _mm256_mul_pd(dZ[1][2], dZ[1][1])));
            const __m256d dZtdZ_2_2 = _mm256_and_pd(valid_mask, _mm256_add_pd(_mm256_mul_pd(dZ[0][2], dZ[0][2]), _mm256_mul_pd(dZ[1][2], dZ[1][2])));
            ...
            Jtr_sum[0] = _mm256_add_pd(Jtr_sum[0], _mm256_add_pd(_mm256_mul_pd(masked_r[0], _mm256_sub_pd(_mm256_mul_pd(X[1], dZ[0][2]), _mm256_mul_pd(X[2], dZ[0][1]))),
                                                    _mm256_mul_pd(masked_r[1], _mm256_sub_pd(_mm256_mul_pd(X[1], dZ[1][2]), _mm256_mul_pd(X[2], dZ[1][1])))));


            Jtr_sum[1] = _mm256_sub_pd(Jtr_sum[1], _mm256_add_pd(_mm256_mul_pd(masked_r[0], _mm256_sub_pd(_mm256_mul_pd(X[0], dZ[0][2]), _mm256_mul_pd(X[2], dZ[0][0]))),
                                                    _mm256_mul_pd(masked_r[1], _mm256_sub_pd(_mm256_mul_pd(X[0], dZ[1][2]), _mm256_mul_pd(X[2], dZ[1][0])))));

            Jtr_sum[2] = _mm256_add_pd(Jtr_sum[2], _mm256_add_pd(_mm256_mul_pd(masked_r[0], _mm256_sub_pd(_mm256_mul_pd(X[0], dZ[0][1]), _mm256_mul_pd(X[1], dZ[0][0]))),
                                                    _mm256_mul_pd(masked_r[1], _mm256_sub_pd(_mm256_mul_pd(X[0], dZ[1][1]), _mm256_mul_pd(X[1], dZ[1][0])))));




            Jtr_sum[3] = _mm256_add_pd(Jtr_sum[3], _mm256_add_pd(_mm256_mul_pd(dZ[0][0], masked_r[0]), _mm256_mul_pd(dZ[1][0], masked_r[1])));
            Jtr_sum[4] = _mm256_add_pd(Jtr_sum[4], _mm256_add_pd(_mm256_mul_pd(dZ[0][1], masked_r[0]), _mm256_mul_pd(dZ[1][1], masked_r[1])));
            Jtr_sum[5] = _mm256_add_pd(Jtr_sum[5], _mm256_add_pd(_mm256_mul_pd(dZ[0][2], masked_r[0]), _mm256_mul_pd(dZ[1][2], masked_r[1])));
        }
...
double CameraJacobianAccumulatorSIMD<NullCameraModel, TruncatedLoss, UniformWeightVector>::residual(const CameraPose &pose) const {
    ...
    for (size_t k = K - K % 4; k < K; ++k) {
        ...
        const double r_squared = (Z.block<2, 1>(0, 0) * inv_z
                                 -x.row(k).transpose()).squaredNorm();
