...
// Estimate two different focal lengths from F using the direct version of Bougnoux formula from
// Rybkin, "Robust Focal Length Computation" (CTU Bachelor Thesis 2017)
std::pair<Camera, Camera> focals_from_fundamental_direct(const Eigen::Matrix3d &F, const Point2D &pp1,
                                                         const Point2D &pp2, double tol=1e-8);
...
focals_from_fundamental_iterative(const Eigen::Matrix3d &F, const Camera &camera1_prior,
                                                         const Camera &camera2_prior, const int &max_iters = 50,
