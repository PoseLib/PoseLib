...
int relpose_7pt(const std::vector<Vector3> &x1, const std::vector<Vector3> &x2,
    ...
    Eigen::Matrix<real, 9, 7> epipolar_constraints;
    ...
    Eigen::Matrix<real, 9, 2> N = Q.rightCols(2);

    // coefficients for det(F(x)) = 0
    const real c3 = N(0, 0) * N(4, 0) * N(8, 0) - N(0, 0) * N(5, 0) * N(7, 0) - N(1, 0) * N(3, 0) * N(8, 0) +
                    N(1, 0) * N(5, 0) * N(6, 0) + N(2, 0) * N(3, 0) * N(7, 0) - N(2, 0) * N(4, 0) * N(6, 0);
    const real c2 = N(0, 0) * N(4, 0) * N(8, 1) + N(0, 0) * N(4, 1) * N(8, 0) - N(0, 0) * N(5, 0) * N(7, 1) -
    ...
    const real c1 = N(0, 0) * N(4, 1) * N(8, 1) - N(0, 0) * N(5, 1) * N(7, 1) + N(0, 1) * N(4, 0) * N(8, 1) +
    ...
    const real c0 = N(0, 1) * N(4, 1) * N(8, 1) - N(0, 1) * N(5, 1) * N(7, 1) - N(1, 1) * N(3, 1) * N(8, 1) +
    ...
    for (int i = 0; i < n_roots; ++i) {
        Eigen::Matrix<real, 9, 1> f = N.col(0) * roots[i] + N.col(1);
        f.normalize();
        fundamental_matrices->push_back(Eigen::Map<Matrix3x3>(f.data()));
    }
