...
int relpose_7pt(const std::vector<Eigen::Vector3_t> &x1, const std::vector<Eigen::Vector3_t> &x2,
    ...
    Eigen::Matrix<real_t, 9, 7> epipolar_constraints;
    ...
    Eigen::Matrix<real_t, 9, 2> N = Q.rightCols(2);

    // coefficients for det(F(x)) = 0
    const real_t c3 = N(0, 0) * N(4, 0) * N(8, 0) - N(0, 0) * N(5, 0) * N(7, 0) - N(1, 0) * N(3, 0) * N(8, 0) +
                      N(1, 0) * N(5, 0) * N(6, 0) + N(2, 0) * N(3, 0) * N(7, 0) - N(2, 0) * N(4, 0) * N(6, 0);
    const real_t c2 = N(0, 0) * N(4, 0) * N(8, 1) + N(0, 0) * N(4, 1) * N(8, 0) - N(0, 0) * N(5, 0) * N(7, 1) -
    ...
    const real_t c1 = N(0, 0) * N(4, 1) * N(8, 1) - N(0, 0) * N(5, 1) * N(7, 1) + N(0, 1) * N(4, 0) * N(8, 1) +
    ...
    const real_t c0 = N(0, 1) * N(4, 1) * N(8, 1) - N(0, 1) * N(5, 1) * N(7, 1) - N(1, 1) * N(3, 1) * N(8, 1) +
                      N(1, 1) * N(5, 1) * N(6, 1) + N(2, 1) * N(3, 1) * N(7, 1) - N(2, 1) * N(4, 1) * N(6, 1);

    // Solve the cubic
    real_t inv_c3 = 1.0 / c3;
    real_t roots[3];
    ...
    for (int i = 0; i < n_roots; ++i) {
        Eigen::Matrix<real_t, 9, 1> f = N.col(0) * roots[i] + N.col(1);
        f.normalize();
        fundamental_matrices->push_back(Eigen::Map<Eigen::Matrix3_t>(f.data()));
    }
