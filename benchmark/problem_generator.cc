...
double CalibPoseValidator::compute_pose_error(
...
double CalibPoseValidator::compute_pose_error(
    const RelativePoseProblemInstance& instance, const CameraPose& pose) {
  return (instance.pose_gt.R() - pose.R()).norm() +
         (instance.pose_gt.t - pose.t).norm();
}

double CalibPoseValidator::compute_pose_error(
    const RelativePoseProblemInstance& instance, const ImagePair& image_pair) {
  return (instance.pose_gt.R() - image_pair.pose.R()).norm() +
         (instance.pose_gt.t - image_pair.pose.t).norm() +
         std::abs(instance.focal_gt - image_pair.camera1.focal()) /
             instance.focal_gt +
         std::abs(instance.focal_gt - image_pair.camera2.focal()) /
             instance.focal_gt;
}

bool CalibPoseValidator::is_valid(const AbsolutePoseProblemInstance& instance,
                                  const CameraPose& pose,
                                  double scale,
                                  double tol) {
  ...
  for (int i = 0; i < instance.x_point_.size(); ++i) {
    double err = 1.0 - std::abs(instance.x_point_[i].dot(
                           (pose.R() * instance.X_point_[i] + pose.t -
                            scale * instance.p_point_[i])
                               .normalized()));
  ...
  for (int i = 0; i < instance.x_line_.size(); ++i) {
    ...
    Eigen::Vector3d X =
        pose.R() * instance.X_line_[i] + pose.t - scale * instance.p_line_[i];
    double err = instance.x_line_[i]
                     .cross(pose.R() * instance.V_line_[i])
                     .normalized()
                     .dot(X);
  ...
  for (int i = 0; i < instance.l_line_point_.size(); ++i) {
    Eigen::Vector3d X = pose.R() * instance.X_line_point_[i] + pose.t -
                        scale * instance.p_line_point_[i];
  ...
  for (int i = 0; i < instance.l_line_line_.size(); ++i) {
    Eigen::Vector3d X = pose.R() * instance.X_line_line_[i] + pose.t -
                        scale * instance.p_line_line_[i];
...
bool CalibPoseValidator::is_valid(const RelativePoseProblemInstance& instance,
                                  const CameraPose& pose,
                                  double tol) {
  if ((pose.R().transpose() * pose.R() - Eigen::Matrix3d::Identity()).norm() >
      tol)
    return false;
...
bool CalibPoseValidator::is_valid(const RelativePoseProblemInstance& instance,
                                  ...
                                  double tol) {
  if ((image_pair.pose.R().transpose() * image_pair.pose.R() -
       Eigen::Matrix3d::Identity())
          .norm() > tol)
    return false;

  Eigen::Matrix3d K_1_inv, K_2_inv;
  K_1_inv << 1.0 / image_pair.camera1.focal(), 0.0, 0.0, 0.0,
      1.0 / image_pair.camera1.focal(), 0.0, 0.0, 0.0, 1.0;
  K_2_inv << 1.0 / image_pair.camera2.focal(), 0.0, 0.0, 0.0,
      1.0 / image_pair.camera2.focal(), 0.0, 0.0, 0.0, 1.0;
  ...
  for (int i = 0; i < instance.x1_.size(); ++i) {
    ...
    double err = std::abs(
        (x2_u.cross(image_pair.pose.R() * x1_u).dot(-image_pair.pose.t)));
    if (err > tol) return false;
  }

  // return is_valid(instance, image_pair.pose, tol) &&
  // (std::fabs(image_pair.camera.focal() - instance.focal_gt) < tol);
  return true;
}

double HomographyValidator::compute_pose_error(
    const RelativePoseProblemInstance& instance, const Eigen::Matrix3d& H) {
...
bool HomographyValidator::is_valid(const RelativePoseProblemInstance& instance,
                                   const Eigen::Matrix3d& H,
                                   double tol) {
  for (int i = 0; i < instance.x1_.size(); ++i) {
    Eigen::Vector3d z = H * instance.x1_[i];
    double err =
        1.0 - std::abs(z.normalized().dot(instance.x2_[i].normalized()));
    if (err > tol) return false;
  }
...
double UnknownFocalValidator::compute_pose_error(
    const AbsolutePoseProblemInstance& instance,
    const CameraPose& pose,
    double focal) {
  return (instance.pose_gt.R() - pose.R()).norm() +
...
bool UnknownFocalValidator::is_valid(
    const AbsolutePoseProblemInstance& instance,
    const CameraPose& pose,
    double focal,
    double tol) {
...
double RadialPoseValidator::compute_pose_error(
    const AbsolutePoseProblemInstance& instance,
    const CameraPose& pose,
    double scale) {
...
bool RadialPoseValidator::is_valid(const AbsolutePoseProblemInstance& instance,
                                   const CameraPose& pose,
                                   double scale,
                                   double tol) {
  if ((pose.R().transpose() * pose.R() - Eigen::Matrix3d::Identity()).norm() >
  ...
  // alpha * p + lambda*x = R*X + t
  for (int i = 0; i < instance.x_point_.size(); ++i) {
    Eigen::Vector3d radial_line{
        -instance.x_point_[i](1), instance.x_point_[i](0), 0.0};
...
void generate_abspose_problems(
    int n_problems,
  ...
  std::uniform_real_distribution<double> depth_gen(options.min_depth_,
                                                   options.max_depth_);
  std::uniform_real_distribution<double> coord_gen(-fov_scale, fov_scale);
  std::uniform_real_distribution<double> scale_gen(options.min_scale_,
                                                   options.max_scale_);
  std::uniform_real_distribution<double> focal_gen(options.min_focal_,
                                                   options.max_focal_);
  ...
  for (int i = 0; i < n_problems; ++i) {
    ...
    for (int j = 0; j < options.n_point_point_; ++j) {
      ...
      Eigen::Vector3d x{
          coord_gen(random_engine), coord_gen(random_engine), 1.0};
      x.normalize();
      Eigen::Vector3d X;

      if (options.generalized_) {
        p << offset_gen(random_engine), offset_gen(random_engine),
            offset_gen(random_engine);
      }
    ...
    // This generates instances where the same 3D point is observed twice in a
    // generalized camera This is degenerate case for the 3Q3 based gp3p/gp4ps
    // solver unless specifically handled.
    if (options.generalized_ && options.generalized_duplicate_obs_) {
      ...
      instance.x_point_[ind[1]] =
          (instance.pose_gt.R() * instance.X_point_[ind[0]] +
           instance.pose_gt.t - instance.scale_gt * instance.p_point_[ind[1]])
              .normalized();
    }
    ...
    for (int j = 0; j < options.n_point_line_; ++j) {
      ...
      Eigen::Vector3d V{direction_gen(random_engine),
    ...
    for (int j = 0; j < options.n_line_point_; ++j) {
      ...
      if (options.radial_lines_) {
        ...
        // Random line
        l = x.cross(Eigen::Vector3d(direction_gen(random_engine),
                                    direction_gen(random_engine),
                                    direction_gen(random_engine)));
      }
...
void generate_relpose_problems(
    int n_problems,
  ...
  for (int i = 0; problem_instances->size() < n_problems; ++i) {
    ...
    for (int j = 0; j < options.n_point_point_; ++j) {
      ...
      if (options.generalized_) {
        p1 << offset_gen(random_engine), offset_gen(random_engine),
            offset_gen(random_engine);
        p2 << offset_gen(random_engine), offset_gen(random_engine),
            offset_gen(random_engine);
    ...
    if (instance.x1_.size() < options.n_point_point_) continue;

    problem_instances->push_back(instance);
  }
}

void generate_homography_problems(
    int n_problems,
    std::vector<RelativePoseProblemInstance>* problem_instances,
    const ProblemOptions& options) {
  problem_instances->clear();
  ...
  while (problem_instances->size() < n_problems) {
    ...
    // Generate plane
    Eigen::Vector3d n;
    n << direction_gen(random_engine), direction_gen(random_engine),
        direction_gen(random_engine);
    n.normalize();
    ...
    instance.H_gt =
        alpha * instance.pose_gt.R() + instance.pose_gt.t * n.transpose();
    ...
    for (int j = 0; j < options.n_point_point_; ++j) {
      ...
      for (int trials = 0; trials < 10; ++trials) {
        ...
        if (x2h(0) < -fov_scale || x2h(0) > fov_scale || x2h(1) < -fov_scale ||
            x2h(1) > fov_scale) {
          // try to generate another point
          continue;
        }
