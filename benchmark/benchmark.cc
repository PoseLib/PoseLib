...
BenchmarkResult benchmark(int n_problems,
  ...
  for (const AbsolutePoseProblemInstance& instance : problem_instances) {
    ...
    // instance.pose_gt.t << "\n"; std::cout << "gt valid = " <<
    // Solver::validator::is_valid(instance, instance.pose_gt, 1.0, tol) <<
    // "\n";
    for (const CameraPose& pose : solutions) {
      ...
      pose_error =
          std::min(pose_error,
                   Solver::validator::compute_pose_error(instance, pose, 1.0));
    }
    if (pose_error < tol) result.found_gt_pose_++;
  }
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    runtimes.push_back(std::chrono::duration_cast<std::chrono::nanoseconds>(
                           end_time - start_time)
                           .count());
  }
...
template <typename Solver>
BenchmarkResult benchmark_w_extra(int n_problems,
                                  const ProblemOptions& options,
                                  double tol = 1e-6) {
  std::vector<AbsolutePoseProblemInstance> problem_instances;
  generate_abspose_problems(n_problems, &problem_instances, options);
  ...
  for (const AbsolutePoseProblemInstance& instance : problem_instances) {
    ...
    for (size_t k = 0; k < solutions.size(); ++k) {
      ...
      pose_error = std::min(pose_error,
                            Solver::validator::compute_pose_error(
                                instance, solutions[k], extra[k]));
    }
...
template <typename Solver>
BenchmarkResult benchmark_relative(int n_problems,
                                   const ProblemOptions& options,
                                   double tol = 1e-6) {
  ...
  for (const RelativePoseProblemInstance& instance : problem_instances) {
    ...
    for (const typename Solver::Solution& pose : solutions) {
      ...
      // std::cout << "Pose: " << pose.R << "\n" << pose.t << "\n";
      pose_error = std::min(
          pose_error, Solver::validator::compute_pose_error(instance, pose));
    }
...
template <typename Solver>
BenchmarkResult benchmark_homography(int n_problems,
                                     const ProblemOptions& options,
                                     double tol = 1e-6) {
  std::vector<RelativePoseProblemInstance> problem_instances;
  ...
  for (const RelativePoseProblemInstance& instance : problem_instances) {
    ...
    for (const Eigen::Matrix3d& H : solutions) {
      if (Solver::validator::is_valid(instance, H, tol))
        result.valid_solutions_++;
      // std::cout << "Pose: " << pose.R << "\n" << pose.t << "\n";
      hom_error = std::min(hom_error,
                           Solver::validator::compute_pose_error(instance, H));
    }

    if (hom_error < tol) result.found_gt_pose_++;
  }
...
void display_result(const std::vector<poselib::BenchmarkResult>& results) {
  ...
  for (const poselib::BenchmarkResult& result : results) {
    ...
    double valid_sols = result.valid_solutions_ /
                        static_cast<double>(result.solutions_) * 100.0;
...
int main() {
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverP3P_lambdatwist>(1e5, p3p_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverGP3P>(1e4, gp3p_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_w_extra<poselib::SolverGP4PS>(1e4, gp4p_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_w_extra<poselib::SolverP4PF>(1e4, p4pf_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverP2P2PL>(1e3, p2p2pl_opt, tol));

  // P6LP
  poselib::ProblemOptions p6lp_opt = options;
  p6lp_opt.n_line_point_ = 6;
  results.push_back(
      poselib::benchmark<poselib::SolverP6LP>(1e4, p6lp_opt, tol));

  // P5LP Radial
  poselib::ProblemOptions p5lp_radial_opt = options;
  p5lp_radial_opt.n_line_point_ = 5;
  p5lp_radial_opt.radial_lines_ = true;
  results.push_back(poselib::benchmark<poselib::SolverP5LP_Radial>(
      1e5, p5lp_radial_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverP2P1LL>(1e4, p2p1ll_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverP1P2LL>(1e4, p1p2ll_opt, tol));

  // P3LL
  poselib::ProblemOptions p3ll_opt = options;
  p3ll_opt.n_line_line_ = 3;
  results.push_back(
      poselib::benchmark<poselib::SolverP3LL>(1e4, p3ll_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverUP2P>(1e6, up2p_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverUP1P1LL>(1e6, up1p1ll_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverUGP2P>(1e6, ugp2p_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_w_extra<poselib::SolverUGP3PS>(1e5, ugp3ps_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverUP1P2PL>(1e4, up1p2pl_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverUP4PL>(1e4, up4pl_opt, tol));
  ...
  results.push_back(
      poselib::benchmark<poselib::SolverUGP4PL>(1e4, ugp4pl_opt, tol));
  ...
  results.push_back(poselib::benchmark_relative<poselib::SolverRelUpright3pt>(
      1e4, relupright3pt_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_relative<poselib::SolverGenRelUpright4pt>(
          1e4, genrelupright4pt_opt, tol));

  // Relative Pose 8pt
  poselib::ProblemOptions rel8pt_opt = options;
  rel8pt_opt.n_point_point_ = 8;
  results.push_back(
      poselib::benchmark_relative<poselib::SolverRel8pt>(1e4, rel8pt_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_relative<poselib::SolverRel5pt>(1e4, rel5pt_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_relative<poselib::SolverSharedFocalRel6pt>(
          1e4, rel_focal_6pt_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_relative<poselib::SolverRelUprightPlanar2pt>(
          1e4, reluprightplanar2pt_opt, tol));
  ...
  results.push_back(
      poselib::benchmark_relative<poselib::SolverRelUprightPlanar3pt>(
          1e4, reluprightplanar3pt_opt, tol));
  ...
  results.push_back(poselib::benchmark_relative<poselib::SolverGenRel5p1pt>(
      1e4, genrel5p1pt_opt, tol));
  ...
  results.push_back(poselib::benchmark_relative<poselib::SolverGenRel6pt>(
      1e3, genrel6pt_opt, tol));

  // Homograpy (4pt)
  poselib::ProblemOptions homo4pt_opt = options;
  homo4pt_opt.n_point_point_ = 4;
  results.push_back(
      poselib::benchmark_homography<poselib::SolverHomography4pt<false>>(
          1e5, homo4pt_opt, tol));
  results.push_back(
      poselib::benchmark_homography<poselib::SolverHomography4pt<true>>(
          1e5, homo4pt_opt, tol));
