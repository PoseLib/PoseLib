...
struct CalibPoseValidator {
  ...
  static double compute_pose_error(const AbsolutePoseProblemInstance& instance,
  ...
  // Checks if the solution is valid (i.e. is rotation matrix and satisfies
  // projection constraints)
  static bool is_valid(const AbsolutePoseProblemInstance& instance,
                       const CameraPose& pose,
                       double scale,
                       double tol);
...
struct ProblemOptions {
  ...
  int generalized_first_cam_obs_ =
      0;  // how many of the points should from the first camera (relpose only)
